...
<script>
    $(document).ready(function () {
        ...
        $('#left-nav-1-child')
                ...
                .append("<dd><a style='color:white;background: #009688;' href='/admin/page/order-comment'>待评论</a>" + "</dd>")
    ...
    layui.use('table', function(){
        ...
        table.render({
            elem: '#orderTable'
            ,height: 400
            ,url: '/admin/order/status/5' //数据接口
            ,page: true //开启分页
            ,parseData: function(res){ //res 即为原始返回的数据
                for(var r in res.data.content){
                    res.data.content[r].status="待评论";
                    res.data.content[r].totalPrice="￥"+res.data.content[r].totalPrice;
