...
<#include "common/left-nav.ftl"/>
    ...
    <script type="text/html" id="operation">
        ...
        <a class="layui-btn layui-btn-xs" lay-event="check">确认</a>
...
<script>
    $(document).ready(function () {
        ...
        $('#left-nav-1-child')
                .append("<dd><a style='color:white;background: #009688;' href='/admin/page/order-to-deal'>新订单</a>" + "</dd>")
                .append("<dd><a href='/admin/page/order-prepare'>揽件中</a>" + "</dd>")
                .append("<dd><a href='/admin/page/order-transport'>运输中</a>" + "</dd>")
                .append("<dd><a href='/admin/page/order-receive'>待收货</a>" + "</dd>")
    ...
    layui.use('table', function(){
        ...
        table.render({
            elem: '#orderTable'
            ,height: 400
            ,url: '/admin/order/status/1' //数据接口
            ,page: true //开启分页
            ,parseData: function(res){ //res 即为原始返回的数据
                for(var r in res.data.content){
                    res.data.content[r].totalPrice="￥"+parseFloat(res.data.content[r].totalPrice).toFixed(2);
                    if (res.data.content[r].delivery!=null){
                        var toAddress = res.data.content[r].delivery.toAddress;
                        res.data.content[r].toAddressInfo= toAddress.province==null?"":toAddress.province+toAddress.city==null?"":toAddress.city+toAddress.town==null?"":toAddress.town+toAddress.detail==null?"":toAddress.detail;
                    } else{
                        res.data.content[r].toAddressInfo="";
                    }
